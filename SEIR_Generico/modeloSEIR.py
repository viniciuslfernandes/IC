    ...
    for i in range(len(valores)):
        for c in range(1):
            ...
            for (u, v, data) in grafo.edges(data=True):
                weight = data['weight']
                config.add_edge_configuration((u, v), 'threshold',  valores[i][0])
            ...
            comparativos = [0, 29, 59, 89, 119, 149, 179, 209, 239]
            infectados_mes = []
            for idx, j in enumerate(iterations):
                # print(j['status'])
                for x in j['status']:
                    if j['status'][x]==1:
                        cont_infectados +=1
                if idx in comparativos:
                    infectados_mes.append(cont_infectados)
                    cont_infectados = 0
            
            porc_acum = 0
            porcentagem_mes = []
            for idx, infectados in enumerate(infectados_mes):
                porcentagem = ( infectados*100)/tam_population
                porcentagem_mes.append(porcentagem)
            
            print(porcentagem_mes)   
            print(infectados_mes)
            with open('./tabelas_seir/tabela_tempo_classes.csv', 'a') as f:
                f.write(f'{porcentagem_mes}\n')
                f.write(f'{infectados_mes}\n')
